...
def VAR1_compute_derivatives(f, Z, second_order=True):
    
    if any(np.issubdtype(z.dtype, np.integer) for z in Z[:-1]):
        Z = [z.astype(np.float) for z in Z[:-1]] + [Z[-1]]
        warnings.warn("Casting integer inputs to float in order to handle differentiation.")
    Z_0, W_0, q_0 = Z

    # 1st-order derivatives
    df_zt = jacobian(f, argnum=0)
    df_wtp1 = jacobian(f, argnum=1)
    df_q = jacobian(f, argnum=2)
    # Evaluate derivatives at Z_0,Z_1,W_0,q_0
    f_zt = np.array(df_zt(Z_0, W_0, q_0))#.reshape(-1,z.shape[0])
    f_wtp1 = np.array(df_wtp1(Z_0, W_0, q_0))#.reshape(-1,w.shape[0])
    f_q = np.array(df_q(Z_0, W_0, q_0))
    f_dim = f_zt.shape[0]
    ...
    if second_order:
        # 2nd-order derivatives
        df_ztzt = jacobian(df_zt, argnum=0)
        df_ztwtp1 = jacobian(df_zt, argnum=1)
        df_ztq = jacobian(df_zt, argnum=2)
        
        df_wtp1wtp1 = jacobian(df_wtp1, argnum=1)
        df_wtp1q = jacobian(df_wtp1, argnum=2)

        df_qq = jacobian(df_q, argnum=2)

        # Evaluate derivatives at Z_0,Z_1,W_0,q_0
        f_ztzt = df_ztzt(Z_0, W_0, q_0).reshape((f_dim, -1))
        f_ztwtp1 = df_ztwtp1(Z_0, W_0, q_0).reshape((f_dim, -1))
        f_ztq = df_ztq(Z_0, W_0, q_0).reshape((f_dim, -1))

        f_wtp1wtp1 = df_wtp1wtp1(Z_0, W_0, q_0).reshape((f_dim, -1))
        f_wtp1q = df_wtp1q(Z_0, W_0, q_0).reshape((f_dim, -1))

        f_qq = df_qq(Z_0, W_0, q_0).reshape((f_dim, -1))

        second_order_derivatives = {'ztzt': f_ztzt,
                                    'ztwtp1': f_ztwtp1,
                                    'ztq': f_ztq,
                                    'wtp1wtp1': f_wtp1wtp1,
                                    'wtp1q': f_wtp1q,
