...
def createMatrix(upperLims, lowerLims, S, N, dVec, increVec, stateMat, model,bc, betterCP):
    ...
    muC = np.matrix(model['muC'](stateMat))
    sigmaC = np.matrix(model['sigmaC'](stateMat))
    T = model['T']
    dt = model['dt']
...
def computeElasSub(stateMat, model, bc, x0, usePardiso, iparms, betterCP):
    ...
    derivs = np.array([ np.matrix(np.zeros([N, model['T']])) for i in range(numStarts)])
    for i in range(numStarts):
        for n in range(0, 2 * N, 2):
            derivs[i][ int(n / 2),:] = ( (res[i][n,:] - res[i][n + 1, :]) / ( 2 * eps ) ) / exp1s[i]
    ###Compute elasticities (type 1)
    firstType = np.zeros([x0.shape[0], numShocks, model['T']])

    for i in range(numStarts):
        firstType[i,:,:] = np.concatenate([ (func(x0[i])) for func in model['sigmaX']]).T * np.matrix(derivs[i,:,:]) + model['sigmaC'](x0[i]).T
